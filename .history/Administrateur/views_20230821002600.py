...
def update_user_registration(request, user_id):
    error = False
    message = ""
    user = get_object_or_404(get_user_model(), pk=user_id)  # Get the user object or return 404 if not found
    ...
    if request.method == "POST":
        ...
        existing_user = get_user_model().objects.filter(Q(email=email) | Q(username=username)).exclude(pk=user_id).first()
        if existing_user:
            error = True
            message = f"L'email {email} ou le nom d'utilisateur {nom} est déjà utilisé par un utilisateur"
